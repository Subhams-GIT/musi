...
export async function createShareLink(spaceId: string, creatorId: string): Promise<string> {
  const redis = await getRedisClient();
  console.log(process.env.NEXT_AUTH_SECRET)
  const hash = jwt.sign({spaceId:spaceId,creatorId},process.env.NEXT_AUTH_SECRET || "",{expiresIn:'1h'});
